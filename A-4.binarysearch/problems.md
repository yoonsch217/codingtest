...
문제: There is an integer array nums sorted in ascending order (with distinct values). 
...
- 왼쪽이 정렬됐는지 오른쪽이 정렬됐는지를 판단한다.
- 왼쪽이 정렬된 경우, target이 왼쪽에 있으려면 `nums[left] <= target < nums[mid]` 여야한다. 이건 필요충분 조건이기 때문에 나머지의 상황은 오른쪽을 탐색한다.
