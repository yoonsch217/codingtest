...
문제: Given an m x n 2D binary grid grid which represents a map of '1's (land) and '0's (water), return the number of islands.
...
class Solution:
    def numIslands(self, grid: List[List[str]]) -> int:
        """
        union-set 
        for each island, mark each land as its representative land, which works as a root
        For each land, compare all its four directions
        And finally, iterate all lands and get the number of unique roots
        O(N*M)
        """
        ...
        roots = [[None for _ in range(n_cols)] for _ in range(n_rows)]  # (i, j) is the root of that position. if it's not a land, None
        ...
        dirs = [(1, 0), (0, 1)]  # no need to look up all 4 directions

        def find_root(row_idx, col_idx) -> Tuple(int, int):
            if roots[row_idx][col_idx] is None:
                return None
            if roots[row_idx][col_idx] == (row_idx, col_idx):
                return (row_idx, col_idx)
            roots[row_idx][col_idx] = find_root(roots[row_idx][col_idx][0], roots[row_idx][col_idx][1])
            return roots[row_idx][col_idx]
        
        def union(row_idx_1, col_idx_1, row_idx_2, col_idx_2) -> None:
            root_row_1, root_col_1 = find_root(row_idx_1, col_idx_1)
            root_row_2, root_col_2 = find_root(row_idx_2, col_idx_2)
            if root_row_1 == root_row_2 and root_col_1 == root_col_2:
                return
            roots[root_row_1][root_col_1] = (root_row_2, root_col_2)
            return
        ...
        for i in range(n_rows):
            for j in range(n_cols):
                ...
                for dx, dy in dirs:
                    next_i = i + dx
                    next_j = j + dy
                    if not (0 <= next_i < n_rows) or not (0 <= next_j < n_cols):
                        continue
                    if grid[next_i][next_j] != "1":
                        continue
                    union(i, j, next_i, next_j)
        ...
        for i in range(n_rows):
            for j in range(n_cols):
                res = find_root(i, j)
                if res is not None:
                    unique_roots.add(res)
        
        return len(unique_roots)
```

개선점
- 각 land 마다 4방향을 다 볼 필요가 없다. 진행 방향으로만 보면 된다.
- 마지막에 roots 를 다시 훑을 필요가 없다. 초기화할 때 각 land 마다 island_count를 1씩 늘려준다. 이후 union 하면서 두 개가 합쳐지는 땅들이라면 count를 1씩 줄이면 된다.
- root index를 (i, j) 로 관리하는 것보다 i * n_cols + j 로 관리하는 게 효율적이다.
...
class Solution:
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        roots = [[None for _ in range(n_cols)] for _ in range(n_rows)]  # (i, j) is the root of that position. if it's not a land, None
        # roots 도 2차원 말고 1차원으로 바꾸는 게 편할 것 같다.

        # initialize
        for i in range(n_rows):
            for j in range(n_cols):
                if grid[i][j] == '1':
                    roots[i][j] = i * n_cols + j
                    self.count += 1

        dirs = [(1, 0), (0, 1)]  # only look up forward directions

        def find_root(row_idx, col_idx) -> int:
            if roots[row_idx][col_idx] is None:
                return None
            if roots[row_idx][col_idx] == row_idx * n_cols + col_idx:
                return row_idx * n_cols + col_idx
            roots[row_idx][col_idx] = find_root(roots[row_idx][col_idx] // n_cols, roots[row_idx][col_idx] % n_cols)
            return roots[row_idx][col_idx]
        
        def union(row_idx_1, col_idx_1, row_idx_2, col_idx_2) -> None:
            root_1 = find_root(row_idx_1, col_idx_1)
            root_2 = find_root(row_idx_2, col_idx_2)
            if root_1 == root_2:
                return
            roots[root_1 // n_cols][root_1 % n_cols] = root_2
            self.count -= 1
            return
